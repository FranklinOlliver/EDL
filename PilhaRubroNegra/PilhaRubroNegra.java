...
public class PilhaRubroNegra {
    ...
    public void duplicarTamanho() { // Redemensionamentpo da pilha, dobrando a capacidade
    ...
    public void reduzirTamanho() { // Redimensionamento da pilha, reduzindo a capacidade pela metade
        int quantosNegros = this.getTamanhoPilhaNegra();
        int quantosRubros = this.getTamanhoPilhaRubro();
        int tamanhoAntigo = this.getCapacidade();
        String copia[] = new String[capacidade];
    ...
    public String popRubro() {
        ...
        // 🔽 VERIFICA SE DEVE REDUZIR
        int total = this.getTamanhoPilhaNegra() + this.getTamanhoPilhaRubro();
        if (total <= this.capacidade / 3 && this.capacidade > 1) {
            System.out.println("Reduzindo tamanho da pilha...");
            this.reduzirTamanho();
        }
